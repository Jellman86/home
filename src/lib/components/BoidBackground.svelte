<script lang="ts">
    ...
    const _predVel = new THREE.Vector3(); const _scratchV1 = new THREE.Vector3(); const _scratchV2 = new THREE.Vector3();
    const _alignF = new THREE.Vector3(); const _cohF = new THREE.Vector3(); const _sepF = new THREE.Vector3();
    ...
    function animate() {
        ...
        _baseCol.set(color); _whiteCol.set(0xffffff);
        ...
        for (let i = 0; i < boidCount; i++) {
            ...
            if (!Number.isFinite(_position.x)) { _position.set(0,0,50); _velocity.set(0,0,1); }
            positions[idx] = _position.x; positions[idx+1] = _position.y; positions[idx+2] = _position.z;
