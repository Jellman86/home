<script lang="ts">
    ...
    interface Props {
        boidCount?: number; color?: string; backgroundColor?: string; useSkybox?: boolean;
        wireframe?: boolean; predatorColor?: string; showTrails?: boolean; fps?: number;
        isTerminal?: boolean; lastInteractionTime?: number; typingPoint?: {x: number, y: number} | null;
        gitHash?: string;
    }
    ...
    // --- 2. COMPONENT STATE (Reactive) ---
    let debugMode = $state(false);
    let recruitmentLevel = $state(0);
    
    // --- 3. REFS & SIMULATION DATA (Non-Reactive) ---
    ...
    // --- 4. SCRATCH OBJECTS (Zero-Allocation) ---
    ...
    let SPEED_LIMIT = $derived(3.8);
    let VISUAL_RANGE_SQ = 50 * 50; 
    let PROTECTED_RANGE_SQ = 22 * 22;
    let SEPARATION_WEIGHT = $derived(12.0); 
    let ALIGNMENT_WEIGHT = $derived(5.0); 
    ...
    function init() {
        ...
        _predPos.set(0, 0, 50); 
        _predVel.set(0, 0, 1).setLength(PREDATOR_SPEED);
    ...
    function animate() {
        ...
        if (pointLight) {
            if (isTerminal && typingPoint) {
                ...
                pointLight.position.set(mouse.x*100, mouse.y*100, 250); 
            }
        }

        target.set((mouse.x * window.innerWidth) / 20, -(mouse.y * window.innerHeight) / 20, 0);
        
        if (isTerminal && now - lastInteractionTime < 2000) { 
            recruitmentLevel = Math.min(1, recruitmentLevel + 0.0005); 
        } else { 
            recruitmentLevel = Math.max(0, recruitmentLevel - 0.008); 
        }
        ...
        for (let i = 0; i < boidCount; i++) {
            ...
            if (deathTimers[i] > 0) {
                ...
                _tempColor.set(0xff0000).lerp(_whiteCol, Math.sin(t*20)*0.5+0.5);
            ...
            if (isObserver && uiRect) {
                ...
                _diff.x += Math.sin(t*0.5+i)*10; _diff.y += Math.cos(t*0.4+i)*10;
                ...
                if (dSqP < 3000 && dSqP > 0.001) { 
                    _newAccel.add(_scratchV1.set(dxP, dyP, dzP).normalize().multiplyScalar(1.2));
                ...
                if (_position.z > 150) _velocity.z -= 0.8; 
            ...
            if (!Number.isFinite(_position.x)) { 
                _position.set(0,0,50); 
                _velocity.set(0,0,1); 
            }
        ...
        const targetBoidIdx = predTargetIdx * 3;
        const predict = _lookAt.set(
            positions[targetBoidIdx] + velocities[targetBoidIdx] * PREDATOR_PREDICT_T, 
            positions[targetBoidIdx+1] + velocities[targetBoidIdx+1] * PREDATOR_PREDICT_T, 
            positions[targetBoidIdx+2] + velocities[targetBoidIdx+2] * PREDATOR_PREDICT_T
        );
