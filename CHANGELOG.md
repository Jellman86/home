...
### Changed
- **Auspex Background**: Hovering Auspex no longer summons the old tentacled creature. The backdrop becomes the daemon's window: the sky goes out, a dim prompt sits in the left margin, and lines of unreadable output scroll up across the screen. It runs on the mascot's clock, so when the cursor dumps itself as hex the room does too, and during its eclipse the room goes dark and eyes open in the margins. The creature's drawing code is gone; the painter module keeps only the cell painter.
