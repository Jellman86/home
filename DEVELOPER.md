...
- **Structure**: One 512×512 byte texture stores irregular gas emission, dust optical depth and HII knots. The gas and young-star births share bent logarithmic arm ridges. Stars sample optical depth with near/far-side attenuation; the dust is not just a coloured overlay. A warm analytical bulge and faint disc sit below the individual stars. This is an illustrative density-wave model, not an N-body simulation.
...
- **Framing**: The core follows the available space above the card. Phones reserve a small complete-disc sky; minimising the card moves it down and opens a centred, full-size viewing mode. Pixel density is capped at 1.25 on phones and 1.5 on desktop for the offscreen pass.
...
Run `npm run check` and `npm test` before pushing. The 14 pure regression checks cover cadence independence, bounded catch-up, responsive framing and repeatable random fields; CI runs them before the production build. Browser QA must additionally check shader compilation, light/dark palettes, minimising/restoring, mobile, reduced motion and project crossfades. Diagnostics include the galaxy mode, simulation time, star count and render-target size. Renderer draw-call counters describe the final pass, not the aggregate offscreen work.
