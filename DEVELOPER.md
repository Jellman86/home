...
The galaxy shares the flock's renderer, but renders linear light into its own capped-resolution HDR target. A full-screen composite in the main scene applies ACES tone mapping and sRGB output once. Positions use an isotropic NDC projection with fixed tilt and mild perspective. `BoidBackground` crossfades the layers according to `backgroundMode`; the hidden flock takes ten small standby steps per second and returns to full rate when visible, without catch-up or stale trails.
...
- **Structure**: One 512×512 byte texture stores irregular gas emission, dust optical depth, HII knots and diffuse clouds in RGBA. The gas and young-star births share bent logarithmic arm ridges. Stars sample optical depth with near/far-side attenuation; the dust is not just a coloured overlay. A warm analytical bulge and diffuse disc sit below the individual stars. This is an illustrative density-wave model, not an N-body simulation.
- **Clock**: `galaxy-model.ts` owns the fixed 1/60-second step policy (maximum four steps per frame). Stars, ages and gas use the same accumulated time. Hidden layers, reduced motion and suspended-tab gaps do not accumulate a catch-up backlog. Young initial stars are approximately sheared by their seeded age to avoid a startup-only thin ribbon.
- **Framing**: The enlarged galaxy uses only viewport dimensions. It never moves or rescales in response to dragging, scrolling or minimising the panel. Phones reserve 270px of sky. Pixel density is capped at 1.25 on phones and 1.5 on desktop for the offscreen pass.
- **Discoveries**: `createEncounterDirector` in `galaxy-model.ts` permits one encounter at a time: the first after 45–110 seconds of visible dark-sky time, then 120–270 seconds of silence between encounters. Ships take 18 seconds; black holes approach slowly from the empty left margin over 72 seconds, with a 4–6 CSS-pixel radius and a growing 48–72px lens region. Hidden layers, light mode, reduced motion and suspended tabs pause the director. `galaxy-encounters.ts` bends the rendered starlight and adds the small silhouettes before tone mapping, never the HTML UI. It also adds three faint stationary distant galaxies. These are visual effects, not new gravitational bodies. No extra render targets, per-frame geometry, timers or dependencies are introduced.
- **Encounter QA**: Local dev URLs `?encounter=ship` and `?encounter=black-hole` bypass the wait and loop a preview. The query is gated by `import.meta.env.DEV` and has no effect in production. The normal schedule and paths have pure regression tests; diagnostics expose the director's current kind, age and next quiet interval.
- **Lifecycle**: Render-target owners and compute materials are explicitly disposed. Missing float render-target support or failed compute initialization uses the seeded still field with a byte compositing target. This fallback still requires WebGL2. Reduced motion draws on theme, layout and mode changes without advancing the simulation.
...
Run `npm run check` and `npm test` before pushing. The 20 pure regression checks cover cadence independence, bounded catch-up, fixed responsive framing, repeatable random fields, sparse encounter scheduling and slow approach paths; CI runs them before the production build. Browser QA must additionally check shader compilation, light/dark palettes, minimising/restoring, mobile, reduced motion and project crossfades. Diagnostics include the galaxy mode, simulation time, star count, render-target size and encounter status. Renderer draw-call counters describe the final pass, not the aggregate offscreen work.
